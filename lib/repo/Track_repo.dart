...
import 'package:musicapp_credixcotask/model/Api_result_model.dart';
import 'package:musicapp_credixcotask/model/Track_model.dart';
import 'package:musicapp_credixcotask/res/Res.dart';
import 'dart:convert';
...
class TrackRepositoryImpl implements TrackRepository {
  ...
  Future<List<TrackModel>> getTracks() async {
    var response = await http.get(Uri.parse(AppStrings.TracksUrl));
    if (response.statusCode == 200) {
      var data = json.decode(response.body);
      print(data['message']['body']);
      List<TrackModel> tracks =
          ApiResultModel.fromJson(data['message']['body']).tracks;
      print(tracks);
      return tracks;
    } else {
      throw Exception();
  ...
  Future<TrackModel> getParticularTrackInfo(String id) async {
    var response = await http.get(
        Uri.parse(AppStrings.TrackInfoUrl1 + id + AppStrings.TrackInfoUrl2));
    print("Id ---- " + id);
    if (response.statusCode == 200) {
      var data = json.decode(response.body);
      print(data.toString());
      ...
      TrackModel track =
          ApiResultModelTrackInfo.fromJson(data['message']['body']).track;
      return track;
    } else {
      //print(response.body);
      throw Exception();
  ...
  Future<String> getParticularTrackLyrics(String id) async {
    var response = await http
        .get(Uri.parse(AppStrings.TrackLyrics1 + id + AppStrings.TrackLyrics2));
    print("Id ---- " + id);
    if (response.statusCode == 200) {
      var data = json.decode(response.body);
      //print("Data --- 1 " + data);
      //print("Data --- 2 " + data['message']['body']);
      String lyrics =
          ApiResultModelLyrics.fromJson(data['message']['body']).lyrics;
      return lyrics;
    } else {
      //print(response.body);
      throw Exception();
